...
def registerImage(moving_vols,
    ...
    if platform.system() in ["Linux", "Darwin"]:
        reg_cmd_modified = 'ITK_GLOBAL_DEFAULT_NUMBER_OF_THREADS=%s;%s' % (n_cores, reg.cmdline)
        import subprocess
        subprocess.run(reg_cmd_modified,shell=True)
    elif platform.system() == "Windows":
        reg_cmd_modified = 'ITK_GLOBAL_DEFAULT_NUMBER_OF_THREADS=%s;%s' % (n_cores, reg.cmdline)
        import subprocess
        subprocess.check_output(reg_cmd_modified,shell=True)
    else:
